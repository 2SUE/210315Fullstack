<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %> 
	
<div id="container">
	<div> 글번호 : <span id="iboard"></span></div>
	<div> 글제목 : <span id="title"></span></div>
	<div>
		작성자 : <span id="unm"></span> | 
		작성일 : <span id="regdt"></span>
	</div>
	<div id="ctnt"></div>
</div>

<script>
	function ajax(iboard) {
		console.log('iboard : ' + iboard);
		
		const param = { iboard } // iboard : ibaord
		
		const init = {
			method: 'POST',
			body: new URLSearchParams(param)
		}
		
		fetch('/board/detail', init)
		.then(function(res) { 
			return res.json(); 
		})
		.then(function(myJson) { // myJson에 응답이 들어옴 => 객체화
			console.log(myJson);
		
			setData(myJson);
		});
	}
	
	function setData(data) {
		var iboardElem = document.querySelector('#iboard');
		var titleElem = document.querySelector('#title');
		var unmElem = document.querySelector('#unm');
		var regdtElem = document.querySelector('#regdt');
		var ctntElem = document.querySelector('#ctnt');
		
		
		iboardElem.innerText = data.iboard; // innerText : 태그 사이에 값 넣어줌 innerHTML : 태그도 적용됨 
		titleElem.innerText = data.title; 
		unmElem.innerText = data.unm; 
		regdtElem.innerText = data.regdt; 
		ctntElem.innerText = data.ctnt; 
	}
	
	ajax(${param.iboard});
</script>